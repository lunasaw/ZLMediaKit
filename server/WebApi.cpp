...
void installWebApi() {
    ...
    api_regist("/index/api/getStorageSpace",[](API_ARGS_MAP_ASYNC){
        ...
        float diskUsedCapacity = DiskSpaceManager::GetCreate()->getUsedDisSpace(path+"/"+appName) *1024 ;//MB
    ...
    api_regist("/index/hook/on_ip_not_found",[](API_ARGS_JSON){
        //拉流代理函数失败
        InfoL << "/index/hook/on_ip_not_found: " << allArgs["url"] << ",stream:" << allArgs["stream"] << ",app:" << allArgs["app"] << ",type:" << allArgs["type"];
    });
